...
bobshell_encrypt() {
	...
	if [   "$bobshell_encrypt_source_type" = file  ] && [ "$bobshell_encrypt_destination_type" = file ]; then
		if [ "$1" = "$2" ]; then
			...
			bobshell_copy "file:$bobshell_encrypt_temp" "$2"
		...
		bobshell_encrypt_result=$(bobshell_copy "$1" stdout: | bobshell_openssl_enc)
		bobshell_copy var:bobshell_encrypt_result "$2"
...
bobshell_decrypt() {
	...
	bobshell_parse_locator "$1" bobshell_decrypt_source_type      bobshell_decrypt_source_ref
	bobshell_parse_locator "$2" bobshell_decrypt_destination_type bobshell_decrypt_destination_ref
	

	if [   "$bobshell_decrypt_source_type" = file  ] && [ "$bobshell_decrypt_destination_type" = file ]; then
		if [ "$1" = "$2" ]; then
			bobshell_decrypt_temp=$(mktemp)
			bobshell_copy "$1" "file:$bobshell_decrypt_temp"
		...
		bobshell_decrypt_result=$(bobshell_copy "$1" stdout: | { cat; printf '\n'; } | bobshell_openssl_enc -d)
		bobshell_copy var:bobshell_decrypt_result "$2"
